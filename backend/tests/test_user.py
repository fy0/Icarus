...
def test_signup():
    ...
    info['nickname'] = random_str_en(config.USER_NICKNAME_FOR_REG_MAX),
    ret = user.new(info)
    assert 'nickname' not in ret['data'], 'nickname 边界情况（32字符）'

    info['nickname'] = random_str_en(config.USER_NICKNAME_FOR_REG_MAX + 1),
    ret = user.new(info)
    assert 'nickname' in ret['data'], 'nickname 越界（33字符）'

    info['nickname'] = '1' + random_str_en(config.USER_NICKNAME_FOR_REG_MAX - 1)
    ret = user.new(info)
    assert 'nickname' in ret['data'], 'nickname 不能以数字开头'

    info['nickname'] = random_str_en(config.USER_NICKNAME_FOR_REG_MIN),
    ret = user.new(info)
    assert 'nickname' not in ret['data'], 'nickname 边界情况（4字符）'

    info['nickname'] = random_str_en(config.USER_NICKNAME_FOR_REG_MIN - 1),
    ret = user.new(info)
    assert 'nickname' in ret['data'], 'nickname 越界（3字符）'

    info['nickname'] = random_str_cn(config.USER_NICKNAME_CN_FOR_REG_MIN)
    ret = user.new(info)
    assert 'nickname' not in ret['data'], 'nickname 中文名称'

    info['nickname'] = random_str_cn(config.USER_NICKNAME_FOR_REG_MAX)
    ret = user.new(info)
    assert 'nickname' not in ret['data'], 'nickname 中文名称最大长度'

    info['nickname'] = random_str_cn(config.USER_NICKNAME_FOR_REG_MAX + 1)
    ret = user.new(info)
    assert 'nickname' in ret['data'], 'nickname 中文名称越界'

    info['nickname'] = random_str_cn(config.USER_NICKNAME_FOR_REG_MAX) + random.choice(string.ascii_letters)
    ret = user.new(info)
    assert 'nickname' in ret['data'], 'nickname 中文名称越界2'

    if config.USER_NICKNAME_CN_FOR_REG_MIN < config.USER_NICKNAME_FOR_REG_MIN:
        info['nickname'] = random_str_cn(config.USER_NICKNAME_CN_FOR_REG_MIN)[:-1] + random.choice(string.ascii_letters)
        ret = user.new(info)
        assert 'nickname' in ret['data'], 'nickname 达到中文最短长度，但其中含有英文，且不及英文最短长度'

    if config.USER_NICKNAME_FOR_REG_MIN - config.USER_NICKNAME_CN_FOR_REG_MIN > 1:
        info['nickname'] = random_str_cn(config.USER_NICKNAME_CN_FOR_REG_MIN) + random.choice(string.ascii_letters)
        ret = user.new(info)
        assert 'nickname' not in ret['data'], 'nickname 达到中文最短长度，但不及英文最短长度'

    # TODO: 反向的就暂不测试了，后面再写

    info['email'] = random_str_en(config.USER_EMAIL_MAX - 7) + '@' + random_str_en(3) + '.com'
    assert len(info['email']) == config.USER_EMAIL_MAX + 1
    info['nickname'] = random_str_en(config.USER_NICKNAME_FOR_REG_MAX)
    ret = user.new(info)
    assert ret['code'] == RETCODE.FAILED
    assert 'email' in ret['data'], '邮箱长度越界'

    info['email'] = random_str_en(config.USER_EMAIL_MAX - 8) + '@' + random_str_en(3) + '.com'
    assert len(info['email']) == config.USER_EMAIL_MAX
    info['nickname'] = random_str_en(config.USER_NICKNAME_FOR_REG_MAX)
    ret = user.new(info)
