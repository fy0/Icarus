...
class CommentView(BaseCrudUserView):
    model = Comment
    ...
    async def before_insert(self, values_lst: List['SQLValuesToWrite']):
        for values in values_lst:
            ...
            if 'reply_to_cmt_id' in values:
                ...
                c: CommentModel = CommentModel.get_by_pk(rtid.to_bin())
    ...
    async def after_insert(self, values_lst: List['SQLValuesToWrite'], records: List['DataRecord']):
        for record in records:
            ...
            post_number = CommentModel.select().where(CommentModel.related_id == record['related_id'], CommentModel.id <= record['id']).count()
            CommentModel.update(post_number=post_number).where(CommentModel.id == record['id']).execute()
    ...
    async def after_update(self, values: 'SQLValuesToWrite', old_records: List['DataRecord'],
        ...
        for old_record, record in zip(old_records, new_records):
            # 管理日志：修改评论状态
            ManageLogModel.add_by_post_changed(self, 'state', MOP.POST_STATE_CHANGE, POST_TYPES.COMMENT,
